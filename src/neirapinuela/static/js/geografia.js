...
const GeoApp = (() => {
  ...
  async function loadMapAndData() {
    ...
    // Asegurar que el overlay respeta el aspect ratio del mapa
    syncOverlayViewBox();
...
/*
  function setupDragAndDrop() {
    ...
    state.stageEl.addEventListener('drop', (ev) => {
      ...
      // Calcular coordenadas relativas en %
  ...
  function placeMarker({ id, num, xPct, yPct }) {
    ...
    const vb = state.overlay.viewBox.baseVal || { width: 1000, height: 625 };
    const x = (xPct / 100) * vb.width;
    const y = (yPct / 100) * vb.height;
...
function placeMarker({ id, num, xPct, yPct }) {
  ...
  function checkAnswers() {
    ...
    state.items.forEach(it => {
      ...
      // tolerancia en % (diagonal de 100x100 es 141.4; tol típica 2.5–4.0)
      const tol = it.solution?.tol ?? 3.0;
      const d = distancePct(placed, it.solution);
      ...
      // dibujar línea hacia solución
      const vb = state.overlay.viewBox.baseVal || { width: 1000, height: 625 };
      const x1 = (placed.xPct/100) * vb.width;
      const y1 = (placed.yPct/100) * vb.height;
      const x2 = (it.solution.xPct/100) * vb.width;
      const y2 = (it.solution.yPct/100) * vb.height;
...
function toggleSolutions() {
 ...
 /*
  function toggleSolutions() {
    ...
    state.items.forEach(it => {
      if (!it.solution) return;
      const x = (it.solution.xPct/100) * vb.width;
      const y = (it.solution.yPct/100) * vb.height;
      const g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      g.setAttribute('class', 'marker solution');
